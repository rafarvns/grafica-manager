...
exports.showPrinterPreferences = showPrinterPreferences;
...
const devmodeCache_1 = require("./devmodeCache");
...
function runPowerShellFile(scriptPath, scriptArgs, timeoutMs) {
    return new Promise((resolve, reject) => {
        const ps = (0, child_process_1.spawn)('powershell.exe', ['-NoProfile', '-ExecutionPolicy', 'Bypass', '-Sta', '-File', scriptPath, ...scriptArgs], { windowsHide: false });
        let stdout = '';
        let stderr = '';
...
async function ensureScriptOnDisk() {
    ...
    const filePath = path_1.default.join(dir, `printer-dialog-${(0, crypto_1.randomUUID)()}.ps1`);
    await fs_1.promises.writeFile(filePath, PS_SCRIPT, 'utf-8');
    cachedScriptPath = filePath;
    return filePath;
}
const DUPLEX_TO_DEVMODE = {
    simplex: 1,
    duplex: 2,
    duplexlong: 2,
    duplexshort: 3,
};
function buildScriptArgs(printerName, prefill, cachedDevmodePath, outputDevmodePath) {
    const orientation = prefill?.orientation === 'landscape' ? 2
        : prefill?.orientation === 'portrait' ? 1
            : 0;
    const copies = prefill?.copies && prefill.copies > 0 ? prefill.copies : 0;
    const color = prefill?.monochrome === true ? 1
...
function parseDriverHash(stderr) {
...
async function writeCacheBlobToTemp(blob) {
    const dir = path_1.default.join(os_1.default.tmpdir(), TEMP_DIR_NAME);
    await fs_1.promises.mkdir(dir, { recursive: true });
    const filePath = path_1.default.join(dir, `devmode-input-${(0, crypto_1.randomUUID)()}.bin`);
    if (blob)
        await fs_1.promises.writeFile(filePath, blob);
    return filePath;
}
function makeOutputBlobPath() {
    return path_1.default.join(os_1.default.tmpdir(), TEMP_DIR_NAME, `devmode-output-${(0, crypto_1.randomUUID)()}.bin`);
}
/**
 ...
 */
async function showPrinterPreferences(printerName, prefill) {
    ...
    try {
        ...
        if (prefill?.quality) {
            try {
                ...
                for (const key of driverHashGuessKeys) {
                    const blob = await (0, devmodeCache_1.loadDevmodeBlob)(key);
                    if (blob) {
        ...
        if (result.stdout.trim())
            console.log('[printerConfig] PS stdout:', result.stdout.trim());
        if (result.stderr.trim())
            console.error('[printerConfig] PS stderr:', result.stderr.trim());
        ...
        if (result.exitCode === 0 && prefill?.quality) {
            ...
            if (driverHash) {
                try {
                    const finalBlob = await fs_1.promises.readFile(outputBlobPath);
                    const cacheKey = (0, devmodeCache_1.buildCacheKey)(safeName, driverHash, prefill.quality);
                    await (0, devmodeCache_1.saveDevmodeBlob)(cacheKey, finalBlob);
        ...
        if (inputBlobPath)
            await safeUnlink(inputBlobPath);
 ...
 */
async function listCacheCandidates(printerName, quality) {
    try {
        const dir = path_1.default.join(electron_1.app.getPath('userData'), 'printer-presets');
        ...
        const stats = await Promise.all(matches.map(async (f) => ({ f, mtime: (await fs_1.promises.stat(path_1.default.join(dir, f))).mtimeMs })));
...
//# sourceMappingURL=printerConfig.js.map